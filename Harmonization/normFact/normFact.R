...
#    - X :(matrix p*n) features*samples matrix to normalize 
...
#  Outputs:
#    
#    - Xn : matrix p*n, normalized version of  X    
...
#    Renard E., Branders S. and Absil P.-A.: Independent Component Analysis to Remove Batch Effects from Merged Microarray Datasets (WABI2016)
   ...
   if (!is.null(ref2)){
      ...
      for (i in 1:length(refType2)){
         ...
         idx_2 = c(idx_2,which(factR2_2$allpv[,i]<0.05))
      }
      
     
      idx2keep =intersect(idx,idx_2)
      print(paste("Keeping",length(idx2keep), "cmpts significantly correlated with covariate"))
   ...
   print(paste("Removing",length(idx),"cmpts significantly correlated with batch"))
   Xn = X - bestcmptA%*% t(bestcmptB)
   Xb = bestcmptA%*% t(bestcmptB)
  ...
  return(list(Xn=Xn,R2=R2,bestSV =bestcmptB,A=A,B=B,Xb=Xb))
